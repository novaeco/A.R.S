...
- `components/touch` + `touch_orient` + `touch_transform` gèrent GT911, l'orientation (swap/mirror) et, si une calibration existe déjà en NVS, l'affine associée.

## Orchestration UI (components/ui)
- `ui.c` : vérifie le flag NVS `setup_done`, initialise thème/gestionnaire/navigation. Si première mise en route : lance directement l'assistant (`ui_wizard_start`), sinon charge le Dashboard.
...
- `ui_wizard` : assistant d’onboarding réduit (Wi-Fi -> succès/erreur) ; marque `setup_done` en NVS et bascule vers le Dashboard.
...
- `touch_orient` applique swap/mirror et sauvegarde en NVS ; `touch_transform` charge une éventuelle affine stockée (sinon identité) et remet swap/mirror à false après injection dans le driver (pour éviter double inversion).
...
- Le flag de setup absent -> assistant lancé : `ui_wizard` démarre par le Wi-Fi. En cas d'échec, l'utilisateur peut passer en mode hors ligne ; en cas de succès, le flag `setup_done` est posé puis le Dashboard est chargé.
...
1. Provisionnement Wi-Fi : en cas d’échec répété, utiliser le bouton "Passer" pour marquer le setup comme terminé et vérifier le dashboard.
...
3. Touch non converti : si l’orientation réelle diffère (écran inversé mécaniquement), swap/mirror doivent être ajustés dans `touch_orient` (ou via stockage préchargé). Les indicateurs d’orientation affichés (`Orientation actuelle`) doivent suivre vos gestes.
...
2. Flash + monitor (avec carte insérée) :
  ...
  - `ui_wizard: Starting Setup Wizard` suivi des transitions Wi-Fi/succès/erreur.
3. Navigation : l'assistant Wi-Fi doit déboucher sur un écran de succès (ou d’erreur avec options Réessayer/Passer) puis charger le Dashboard. La touche retour doit revenir au Dashboard (`ui_nav_go_back`).
4. Rollback simple : supprimer le namespace NVS `ui_setup` pour relancer l’assistant (via `nvs_erase` ou `esptool.py erase_region` ciblé), ou re-flasher avec `idf.py -p COM3 erase_flash flash`.
