...
#if CONFIG_ESP_TLS_USE_DEFAULT_CERTIFICATE_BUNDLE
...
static void iot_ota_task(void *param) {
  ...
  if (http_cfg.crt_bundle_attach == NULL) {
    ESP_LOGW(TAG,
             "OTA: aucun bundle de certificats. Activez CONFIG_ESP_TLS_USE_"
             "DEFAULT_CERTIFICATE_BUNDLE pour une vérification TLS par défaut");
  }
